    ...
    if (result) {
      ...
      return (
        <div className="text-center text-gray-600 flex flex-col items-center gap-6 animate-fade-in">
          <CheckCircleIcon />
          <h3 className="text-2xl font-bold text-gray-900">Kompresi Selesai!</h3>
          <div className="flex justify-around w-full max-w-sm bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm">
            <div>
              <p className="text-sm text-gray-500">Ukuran Asli</p>
              <p className="text-lg font-semibold text-gray-800">{formatBytes(result.originalSize)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Ukuran Baru</p>
              <p className="text-lg font-semibold text-gray-800">{formatBytes(result.newSize)}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">{savings >= 0 ? 'Hemat' : 'Bertambah'}</p>
              <p className={`text-lg font-bold ${savings >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {Math.abs(savings).toFixed(1)}%
              </p>
            </div>
          </div>
          <a href={result.url} download={`${fileWithBuffer?.file.name.replace('.pdf', '')}-dikompres.pdf`} className="flex items-center justify-center gap-3 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-300 text-lg shadow-md">
            <DownloadIcon />
            Unduh PDF
          </a>
    ...
    if (fileWithBuffer) {
      return (
        <div className="animate-fade-in">
          <div className="bg-gray-50 border border-gray-200 p-4 rounded-lg flex items-center justify-between mb-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-gray-500 text-sm">{formatBytes(fileWithBuffer.file.size)}</p>
              </div>
            </div>
          ...
          <h3 className="text-lg font-semibold text-center text-gray-800 mb-4">Pilih Mode Kompresi</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <label onClick={() => setCompressionMode('recommended')} className={`p-4 rounded-xl cursor-pointer border-2 transition-all text-left flex flex-col justify-between ${compressionMode === 'recommended' ? 'bg-blue-50 border-blue-500 shadow-sm' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                <div className="flex items-center">
                    <input type="radio" name="compression" value="recommended" checked={compressionMode === 'recommended'} onChange={() => {}} className="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2" />
                    <div className="ml-3">
                        <span className="block font-bold text-gray-800">Kompresi Direkomendasikan</span>
                        <span className="text-xs text-gray-500">Keseimbangan terbaik antara ukuran dan kualitas.</span>
                    </div>
                </div>
                  ...
                  {isEstimating ? (
                    ...
                    <p className="text-gray-600">
                      ...
                      {fileWithBuffer && estimatedSize < fileWithBuffer.file.size ?
                        <span className="text-green-600 ml-2">(-{(100 - (estimatedSize / fileWithBuffer.file.size) * 100).toFixed(1)}%)</span> :
             ...
             <label onClick={() => setCompressionMode('advanced')} className={`p-4 rounded-xl cursor-pointer border-2 transition-all text-left ${compressionMode === 'advanced' ? 'bg-blue-50 border-blue-500 shadow-sm' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                <div className="flex items-center">
                    <input type="radio" name="compression" value="advanced" checked={compressionMode === 'advanced'} onChange={() => {}} className="w-5 h-5 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2" />
                    <div className="ml-3">
                        <span className="block font-bold text-gray-800">Kompresi Lanjutan</span>
                        <span className="text-xs text-gray-500">Pilih ukuran file target yang Anda inginkan (kualitas mungkin berkurang).</span>
                    </div>
                </div>
            </label>
          </div>
          ...
          {compressionMode === 'advanced' && (
            <div className="bg-gray-50 p-4 rounded-lg mb-8 animate-fade-in border border-gray-200">
                <label htmlFor="target-size" className="block mb-2 text-sm font-medium text-gray-700">Ukuran File Target</label>
                <div className="flex">
                    <input 
                        type="number" 
                        id="target-size" 
                        ...
                        className="bg-white border border-gray-300 text-gray-900 text-sm rounded-l-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 shadow-sm" 
                        placeholder="e.g., 500" 
                    />
                    <select 
                        value={targetUnit}
                        onChange={(e) => setTargetUnit(e.target.value as 'KB' | 'MB')}
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-r-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 border-l-0"
                    >
                        <option value="KB">KB</option>
                        <option value="MB">MB</option>
                    </select>
                </div>
                 <p className="text-xs text-gray-500 mt-2">Aplikasi akan mencoba mengompres sedekat mungkin ke ukuran ini.</p>
            </div>
          )}

          <div className="flex justify-center">
            <button onClick={handleCompress} disabled={isProcessing} className="w-full md:w-auto bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed text-white font-bold py-3 px-8 rounded-lg transition-colors text-lg flex items-center justify-center min-w-[200px] shadow-md">
              {isProcessing ? (
                <>
