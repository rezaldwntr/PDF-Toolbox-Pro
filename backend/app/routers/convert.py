...
def convert_pdf_to_docx(
    ...
    try:
        ...
        # Hitung core CPU server untuk akselerasi multi-proses paralel
...
def convert_pdf_to_excel(
    background_tasks: BackgroundTasks,
    file: UploadFile = File(...),
    mode: str = Form("tables_only"),  # "tables_only" (default bersih) atau "all_content"
    ...
    try:
        ...
        with pd.ExcelWriter(tmp_xlsx_path, engine='openpyxl') as writer:
            ...
            def get_page_tables_and_text(page_idx):
                ...
                # Coba deteksi tabel cepat dengan PyMuPDF find_tables (C++ Engine)
                ...
                # Fallback ke pdfplumber jika fitz belum menemukan tabel
                if not tables_data:
                    try:
                ...
                # Ambil teks terstruktur per baris/blok (HANYA jika pengguna memilih mode all_content)
                text_lines = []
                if mode == "all_content":
                    ...
                    for b in blocks:
                        if len(b) > 4 and b[6] == 0:
                            ...
                            if cleaned_block:
                                text_lines.append(cleaned_block)

                return tables_data, text_lines

            if sheet_per_page:
                for page_idx in range(num_pages):
                    sheet_name = f"Halaman {page_idx + 1}"
                ...
                for page_idx in range(num_pages):
                    ...
                    if num_pages > 1 and (tables or (mode == "all_content" and text_lines)):
                        page_label_cell = ws.cell(row=cur_row, column=1, value=f"=== HALAMAN {page_idx + 1} ===")
                        page_label_cell.font = Font(bold=True, color='1E40AF')
                        cur_row += 2
...
def convert_pdf_to_ppt(
    background_tasks: BackgroundTasks,
    file: UploadFile = File(...),
    layout_mode: str = Form("editable"),  # "editable" (kelompokkan per paragraf/blok) atau "visual" (pertahankan background & grafis slide)
):
    validate_file(file)
    safe_basename = sanitize_filename(os.path.splitext(file.filename)[0])
    tmp_dir = tempfile.mkdtemp()
    ...
    try:
        ...
        for i, page in enumerate(doc):
            ...
            if layout_mode == "visual":
                # Mode Presisi Visual: Merender layout grafis, background warna & ornamen slide beresolusi tinggi
                ...
                # Overlay teks yang dapat disalin & diedit
                ...
                # Mode Editable Standar: Ekstrak foto/gambar + kelompokkan teks per BLOK PARAGRAF (bukan per baris terfragmentasi)
        ...
        cleanup_folder(tmp_dir)
        logging.error(f"PPT error: {e}")
        raise HTTPException(status_code=500, detail=f"Gagal convert PPT: {str(e)}")


# === 4. PDF KE IMAGE (Zero Disk I/O In-Memory Streaming) ===
@router.post("/pdf-to-image")
def convert_pdf_to_image(
    file: UploadFile = File(...),
    output_format: str = Form("jpg"),
    dpi: int = Form(150),
    extract_mode: str = Form("pages"),  # "pages" (tiap halaman jadi gambar) atau "embedded" (ekstrak foto di dalam PDF)
):
    validate_file(file)
    safe_basename = sanitize_filename(os.path.splitext(file.filename)[0])
    ...
    try:
        ...
        # MODE 1: Ekstrak Gambar/Foto Tertanam (Extract Embedded Images Only)
        ...
        # MODE 2: Konversi Halaman ke Gambar (Page to Image)
        # Jika hanya 1 halaman: Kirim file gambar langsung tanpa ZIP!
        if num_pages == 1:
            page = doc[0]
            ...
            return Response(
                ...
                headers={"Content-Disposition": f'attachment; filename="{single_filename}"'}
            )

        # Jika Multi-Halaman: In-Memory Zero Disk I/O Streaming ke ZIP
        zip_buffer = io.BytesIO()
        with ZipFile(zip_buffer, 'w', compression=ZIP_DEFLATED) as zipf:
            for i, page in enumerate(doc):
                pix = page.get_pixmap(dpi=target_dpi)
        ...
        raise HTTPException(status_code=500, detail=f"Gagal convert Gambar: {str(e)}")
